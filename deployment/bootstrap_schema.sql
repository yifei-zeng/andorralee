-- 完整初始化脚本：创建 andorralee 数据库以及项目所需全部表/视图
-- 运行方式：mysql -uroot -p < ./deployment/bootstrap_schema.sql

CREATE DATABASE IF NOT EXISTS andorralee DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE andorralee;

/* ========================= 基础管理表 ========================= */
CREATE TABLE IF NOT EXISTS honeypot_template (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    name VARCHAR(50) NOT NULL COMMENT '模板名称',
    protocol VARCHAR(20) NOT NULL COMMENT '协议类型',
    import_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '导入时间',
    deploy_count INT DEFAULT 0 COMMENT '已部署数量',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_template_name (name)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='蜜罐模板管理';

CREATE TABLE IF NOT EXISTS honeypot_instance (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    template_id BIGINT UNSIGNED DEFAULT NULL COMMENT '关联模板ID',
    name VARCHAR(50) NOT NULL COMMENT '实例名称',
    honeypot_name VARCHAR(100) NOT NULL COMMENT '蜜罐名称',
    container_name VARCHAR(50) NOT NULL COMMENT '容器名称',
    container_id VARCHAR(64) DEFAULT NULL COMMENT 'Docker容器ID',
    ip VARCHAR(45) NOT NULL COMMENT 'IP地址',
    honeypot_ip VARCHAR(45) DEFAULT NULL COMMENT '蜜罐IP地址',
    port INT NOT NULL COMMENT '端口号',
    protocol VARCHAR(20) NOT NULL COMMENT '协议类型',
    interface_type VARCHAR(50) DEFAULT NULL COMMENT '蜜罐接口类型',
    status VARCHAR(20) NOT NULL DEFAULT 'created' COMMENT '部署状态',
    image_name VARCHAR(200) DEFAULT NULL COMMENT 'Docker镜像名称',
    image_id VARCHAR(64) DEFAULT NULL COMMENT 'Docker镜像ID',
    port_mappings JSON DEFAULT NULL COMMENT '端口映射配置',
    environment JSON DEFAULT NULL COMMENT '环境变量配置',
    description TEXT COMMENT '描述',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    KEY idx_template_id (template_id),
    KEY idx_container_id (container_id),
    KEY idx_image_id (image_id),
    KEY idx_status (status),
    KEY idx_honeypot_name (honeypot_name),
    CONSTRAINT fk_instance_template FOREIGN KEY (template_id) REFERENCES honeypot_template(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='蜜罐实例管理';

CREATE TABLE IF NOT EXISTS honeypot_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    instance_id BIGINT UNSIGNED NOT NULL COMMENT '关联的蜜罐实例ID',
    log_type VARCHAR(20) NOT NULL COMMENT '日志类型',
    content TEXT NOT NULL COMMENT '日志内容',
    log_time DATETIME(3) NOT NULL COMMENT '记录时间',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (id),
    KEY idx_instance_id (instance_id),
    KEY idx_log_time (log_time),
    CONSTRAINT fk_honeypot_log_instance FOREIGN KEY (instance_id) REFERENCES honeypot_instance(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='蜜罐运行日志';

CREATE TABLE IF NOT EXISTS bait (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    name VARCHAR(50) NOT NULL COMMENT '诱饵名称',
    file_type VARCHAR(10) NOT NULL COMMENT '文件类型',
    is_deployed TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已部署',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    instance_id BIGINT UNSIGNED DEFAULT NULL COMMENT '部署的蜜罐实例ID',
    PRIMARY KEY (id),
    KEY idx_instance_id (instance_id),
    CONSTRAINT fk_bait_instance FOREIGN KEY (instance_id) REFERENCES honeypot_instance(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='诱饵管理';

CREATE TABLE IF NOT EXISTS security_rule (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    rule_name VARCHAR(50) NOT NULL COMMENT '规则名称',
    trigger_conditions TEXT NOT NULL COMMENT '触发条件',
    actions TEXT NOT NULL COMMENT '执行动作',
    is_enabled TINYINT(1) NOT NULL DEFAULT 1 COMMENT '启用状态',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='安全规则管理';

CREATE TABLE IF NOT EXISTS rule_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    rule_id BIGINT UNSIGNED NOT NULL COMMENT '关联的规则ID',
    rule_name VARCHAR(50) NOT NULL COMMENT '规则名称',
    content TEXT NOT NULL COMMENT '执行内容',
    log_time DATETIME(3) NOT NULL COMMENT '执行时间',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入时间',
    PRIMARY KEY (id),
    KEY idx_rule_id (rule_id),
    CONSTRAINT fk_rule_log_rule FOREIGN KEY (rule_id) REFERENCES security_rule(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='规则执行日志';

/* ========================= Docker 及容器管理 ========================= */
CREATE TABLE IF NOT EXISTS docker_image (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    image_id VARCHAR(64) NOT NULL COMMENT '镜像ID',
    repository VARCHAR(100) DEFAULT NULL COMMENT '仓库名称',
    tag VARCHAR(50) DEFAULT NULL COMMENT '标签',
    digest VARCHAR(100) DEFAULT NULL COMMENT '摘要',
    size BIGINT DEFAULT NULL COMMENT '镜像大小(字节)',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_image_id (image_id),
    KEY idx_repository (repository),
    KEY idx_tag (tag)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Docker镜像管理';

CREATE TABLE IF NOT EXISTS docker_image_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    image_id VARCHAR(64) DEFAULT NULL COMMENT '镜像ID',
    image_name VARCHAR(200) DEFAULT NULL COMMENT '镜像名称(含仓库与标签)',
    operation VARCHAR(20) NOT NULL COMMENT '操作类型',
    details TEXT COMMENT '操作详情',
    status VARCHAR(10) NOT NULL COMMENT '操作状态',
    message TEXT COMMENT '状态消息',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (id),
    KEY idx_image_id (image_id),
    KEY idx_operation (operation),
    KEY idx_status (status),
    KEY idx_created_at (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Docker镜像操作日志';

CREATE TABLE IF NOT EXISTS docker_container (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    container_id VARCHAR(64) NOT NULL COMMENT '容器ID',
    container_name VARCHAR(100) NOT NULL COMMENT '容器名称',
    image_id VARCHAR(64) DEFAULT NULL COMMENT '镜像ID',
    image_name VARCHAR(200) DEFAULT NULL COMMENT '镜像名称',
    status VARCHAR(20) DEFAULT NULL COMMENT '容器状态',
    ports JSON DEFAULT NULL COMMENT '端口映射',
    environment JSON DEFAULT NULL COMMENT '环境变量',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_container_id (container_id),
    KEY idx_container_name (container_name),
    KEY idx_image_id (image_id),
    KEY idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Docker容器管理';

CREATE TABLE IF NOT EXISTS container_log_segment (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    container_id VARCHAR(64) NOT NULL COMMENT '容器ID',
    container_name VARCHAR(100) DEFAULT NULL COMMENT '容器名称',
    segment_type VARCHAR(20) NOT NULL COMMENT '日志段类型',
    content TEXT NOT NULL COMMENT '日志内容',
    timestamp DATETIME(3) DEFAULT NULL COMMENT '日志时间戳',
    line_number INT DEFAULT NULL COMMENT '行号',
    component VARCHAR(50) DEFAULT NULL COMMENT '组件名称',
    severity_level VARCHAR(10) DEFAULT NULL COMMENT '严重程度',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '分析时间',
    PRIMARY KEY (id),
    KEY idx_container_id (container_id),
    KEY idx_segment_type (segment_type),
    KEY idx_timestamp (timestamp),
    KEY idx_severity_level (severity_level)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='容器日志语义分析结果';

/* 视图：容器与镜像信息 */
CREATE OR REPLACE VIEW v_container_with_image AS
SELECT 
    c.id,
    c.container_id,
    c.container_name,
    c.status,
    c.ports,
    c.environment,
    c.created_at AS container_created_at,
    i.repository,
    i.tag,
    i.size AS image_size,
    i.created_at AS image_created_at
FROM docker_container c
LEFT JOIN docker_image i ON c.image_id = i.image_id;

CREATE OR REPLACE VIEW v_log_statistics AS
SELECT
    container_id,
    container_name,
    segment_type,
    COUNT(*) AS log_count,
    MIN(timestamp) AS first_log_time,
    MAX(timestamp) AS last_log_time,
    MAX(created_at) AS last_analysis_time
FROM container_log_segment
GROUP BY container_id, container_name, segment_type;

/* ========================= 恶意软件扫描/威胁情报 ========================= */
CREATE TABLE IF NOT EXISTS malware_signature (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL COMMENT '特征名称',
    pattern TEXT NOT NULL COMMENT '特征模式',
    type VARCHAR(20) NOT NULL COMMENT '类型(hash/string/regex)',
    severity VARCHAR(20) NOT NULL COMMENT '严重程度',
    description TEXT COMMENT '描述',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    is_active TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否激活',
    PRIMARY KEY (id),
    UNIQUE KEY uk_signature_name (name)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='病毒特征库';

CREATE TABLE IF NOT EXISTS scan_result (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    file_hash VARCHAR(64) NOT NULL COMMENT '文件SHA256',
    md5_hash VARCHAR(32) DEFAULT NULL COMMENT '文件MD5',
    file_name VARCHAR(255) NOT NULL COMMENT '文件名',
    file_size BIGINT NOT NULL COMMENT '文件大小',
    scan_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '扫描时间',
    is_infected TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否感染',
    threat_level VARCHAR(20) DEFAULT NULL COMMENT '威胁等级',
    detection_count INT DEFAULT 0 COMMENT '威胁数量',
    scan_duration_ms BIGINT DEFAULT NULL COMMENT '扫描耗时',
    source_ip VARCHAR(45) DEFAULT NULL COMMENT '来源IP',
    user_agent TEXT COMMENT '用户代理',
    file_path VARCHAR(500) DEFAULT NULL COMMENT '服务器端保存路径',
    status VARCHAR(20) NOT NULL DEFAULT 'uploaded' COMMENT '扫描状态',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_file_hash (file_hash),
    KEY idx_scan_time (scan_time),
    KEY idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='扫描结果';

CREATE TABLE IF NOT EXISTS detection_result (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    scan_result_id BIGINT UNSIGNED NOT NULL COMMENT '扫描结果ID',
    signature_id BIGINT UNSIGNED DEFAULT NULL COMMENT '特征ID',
    signature_name VARCHAR(100) DEFAULT NULL COMMENT '特征名称',
    match_type VARCHAR(20) DEFAULT NULL COMMENT '匹配类型',
    match_content TEXT COMMENT '匹配内容',
    severity VARCHAR(20) DEFAULT NULL COMMENT '严重程度',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (id),
    KEY idx_scan_result_id (scan_result_id),
    CONSTRAINT fk_detection_scan FOREIGN KEY (scan_result_id) REFERENCES scan_result(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='检测详情';

CREATE TABLE IF NOT EXISTS attack_session (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    session_id VARCHAR(36) NOT NULL COMMENT '会话唯一ID',
    source_ip VARCHAR(45) NOT NULL COMMENT '攻击者IP',
    source_port INT DEFAULT NULL COMMENT '攻击者端口',
    destination_ip VARCHAR(45) NOT NULL COMMENT '目标IP',
    destination_port INT NOT NULL COMMENT '目标端口',
    protocol VARCHAR(20) NOT NULL COMMENT '协议类型',
    start_time DATETIME(3) NOT NULL COMMENT '会话开始时间',
    end_time DATETIME(3) DEFAULT NULL COMMENT '会话结束时间',
    duration BIGINT DEFAULT NULL COMMENT '持续时间(秒)',
    status VARCHAR(20) NOT NULL DEFAULT 'active' COMMENT '会话状态',
    auth_attempts INT DEFAULT 0 COMMENT '认证尝试次数',
    command_count INT DEFAULT 0 COMMENT '命令数',
    threat_level VARCHAR(20) DEFAULT NULL COMMENT '威胁等级',
    container_id VARCHAR(64) DEFAULT NULL COMMENT '容器ID',
    container_name VARCHAR(100) DEFAULT NULL COMMENT '容器名称',
    user_agent TEXT COMMENT '用户代理',
    fingerprint VARCHAR(64) DEFAULT NULL COMMENT '客户端指纹',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_session_id (session_id),
    KEY idx_source_ip (source_ip),
    KEY idx_protocol (protocol)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='攻击会话';

CREATE TABLE IF NOT EXISTS attack_event (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    session_id VARCHAR(36) NOT NULL COMMENT '关联会话ID',
    event_type VARCHAR(30) NOT NULL COMMENT '事件类型',
    event_time DATETIME(3) NOT NULL COMMENT '事件时间',
    source_ip VARCHAR(45) NOT NULL COMMENT '攻击者IP',
    protocol VARCHAR(20) NOT NULL COMMENT '协议',
    username VARCHAR(255) DEFAULT NULL COMMENT '用户名',
    password VARCHAR(255) DEFAULT NULL COMMENT '密码',
    command TEXT COMMENT '执行命令',
    payload TEXT COMMENT '攻击载荷',
    result TEXT COMMENT '执行结果',
    threat_level VARCHAR(20) DEFAULT NULL COMMENT '威胁等级',
    is_blocked TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否被阻断',
    raw_data TEXT COMMENT '原始数据',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (id),
    KEY idx_session_id (session_id),
    KEY idx_event_time (event_time),
    CONSTRAINT fk_attack_event_session FOREIGN KEY (session_id) REFERENCES attack_session(session_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='攻击事件';

CREATE TABLE IF NOT EXISTS threat_intelligence (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    indicator_type VARCHAR(20) NOT NULL COMMENT '指标类型',
    indicator_value VARCHAR(255) NOT NULL COMMENT '指标值',
    threat_type VARCHAR(50) DEFAULT NULL COMMENT '威胁类型',
    confidence INT DEFAULT NULL COMMENT '置信度',
    severity VARCHAR(20) DEFAULT NULL COMMENT '严重程度',
    source VARCHAR(100) DEFAULT NULL COMMENT '情报来源',
    description TEXT COMMENT '描述',
    first_seen DATETIME(3) DEFAULT NULL COMMENT '首次发现时间',
    last_seen DATETIME(3) DEFAULT NULL COMMENT '最后发现时间',
    is_active TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否激活',
    tags JSON DEFAULT NULL COMMENT '标签',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
    KEY idx_indicator_value (indicator_value)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='威胁情报库';

CREATE TABLE IF NOT EXISTS honeytoken_event (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    token_id VARCHAR(36) NOT NULL COMMENT '蜜签ID',
    token_type VARCHAR(50) NOT NULL COMMENT '蜜签类型',
    token_name VARCHAR(100) DEFAULT NULL COMMENT '蜜签名称',
    trigger_time DATETIME(3) NOT NULL COMMENT '触发时间',
    source_ip VARCHAR(45) NOT NULL COMMENT '触发者IP',
    user_agent TEXT COMMENT '用户代理',
    request_path VARCHAR(500) DEFAULT NULL COMMENT '请求路径',
    request_data TEXT COMMENT '请求数据',
    response_code INT DEFAULT NULL COMMENT '响应码',
    location VARCHAR(255) DEFAULT NULL COMMENT '蜜签位置',
    description TEXT COMMENT '描述',
    threat_level VARCHAR(20) DEFAULT NULL COMMENT '威胁等级',
    is_processed TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已处理',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    PRIMARY KEY (id),
    KEY idx_token_id (token_id),
    KEY idx_source_ip (source_ip)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='蜜签事件';

/* ========================= 蜜罐日志：Heralding ========================= */
CREATE TABLE IF NOT EXISTS heralding_auth_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    timestamp DATETIME(6) NOT NULL COMMENT '捕获时间',
    auth_id VARCHAR(36) NOT NULL COMMENT '认证行为ID',
    session_id VARCHAR(36) NOT NULL COMMENT '会话ID',
    source_ip VARCHAR(45) NOT NULL COMMENT '攻击者IP',
    source_port INT UNSIGNED NOT NULL COMMENT '攻击端口',
    destination_ip VARCHAR(45) NOT NULL COMMENT '目标IP',
    destination_port INT UNSIGNED NOT NULL COMMENT '目标端口',
    protocol VARCHAR(20) NOT NULL COMMENT '协议',
    username VARCHAR(255) NOT NULL COMMENT '用户名',
    password VARCHAR(255) NOT NULL COMMENT '密码',
    password_hash VARCHAR(255) DEFAULT NULL COMMENT '密码hash',
    container_id VARCHAR(64) DEFAULT NULL COMMENT '容器ID',
    container_name VARCHAR(100) DEFAULT NULL COMMENT '容器名称',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录创建时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_heralding_auth (auth_id),
    KEY idx_timestamp (timestamp),
    KEY idx_session_id (session_id),
    KEY idx_source_ip (source_ip),
    KEY idx_destination_ip (destination_ip),
    KEY idx_protocol (protocol),
    KEY idx_container_id (container_id),
    KEY idx_username (username)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Heralding认证日志';

CREATE OR REPLACE VIEW v_heralding_auth_statistics AS
SELECT
    DATE(timestamp) AS log_date,
    protocol,
    COUNT(*) AS total_attempts,
    COUNT(DISTINCT source_ip) AS unique_ips,
    COUNT(DISTINCT username) AS unique_usernames,
    COUNT(DISTINCT session_id) AS unique_sessions,
    MIN(timestamp) AS first_attempt,
    MAX(timestamp) AS last_attempt
FROM heralding_auth_log
GROUP BY DATE(timestamp), protocol;

CREATE OR REPLACE VIEW v_attacker_ip_statistics AS
SELECT
    source_ip,
    COUNT(*) AS total_attempts,
    COUNT(DISTINCT protocol) AS protocols_used,
    COUNT(DISTINCT username) AS usernames_tried,
    COUNT(DISTINCT destination_port) AS ports_targeted,
    MIN(timestamp) AS first_seen,
    MAX(timestamp) AS last_seen,
    TIMESTAMPDIFF(MINUTE, MIN(timestamp), MAX(timestamp)) AS attack_duration_minutes
FROM heralding_auth_log
GROUP BY source_ip
ORDER BY total_attempts DESC;

/* ========================= 蜜罐日志：Cowrie ========================= */
CREATE TABLE IF NOT EXISTS cowrie_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    event_time DATETIME(6) NOT NULL COMMENT '事件时间',
    auth_id VARCHAR(36) NOT NULL COMMENT '认证ID',
    session_id VARCHAR(36) NOT NULL COMMENT '会话ID',
    source_ip VARCHAR(45) NOT NULL COMMENT '攻击者IP',
    source_port SMALLINT UNSIGNED NOT NULL COMMENT '攻击端口',
    destination_ip VARCHAR(45) NOT NULL COMMENT '目标IP',
    destination_port SMALLINT UNSIGNED NOT NULL COMMENT '目标端口',
    protocol ENUM('http','ssh','telnet','ftp','smb','other') NOT NULL COMMENT '协议',
    client_info VARCHAR(255) DEFAULT NULL COMMENT '客户端信息',
    fingerprint VARCHAR(64) DEFAULT NULL COMMENT '指纹',
    username VARCHAR(255) DEFAULT NULL COMMENT '用户名',
    password VARCHAR(255) DEFAULT NULL COMMENT '密码',
    password_hash VARCHAR(255) DEFAULT NULL COMMENT '密码hash',
    command TEXT COMMENT '命令',
    command_found TINYINT(1) DEFAULT NULL COMMENT '命令是否识别',
    raw_log TEXT NOT NULL COMMENT '原始日志',
    container_id VARCHAR(64) DEFAULT NULL COMMENT '容器ID',
    container_name VARCHAR(100) DEFAULT NULL COMMENT '容器名称',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录创建时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_cowrie_auth (auth_id),
    KEY idx_event_time (event_time),
    KEY idx_session_id (session_id),
    KEY idx_source_ip (source_ip),
    KEY idx_destination_ip (destination_ip),
    KEY idx_protocol (protocol),
    KEY idx_container_id (container_id),
    KEY idx_username (username),
    KEY idx_command_found (command_found)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Cowrie蜜罐日志';

CREATE OR REPLACE VIEW v_cowrie_statistics AS
SELECT
    DATE(event_time) AS log_date,
    protocol,
    COUNT(*) AS total_events,
    COUNT(DISTINCT source_ip) AS unique_ips,
    COUNT(DISTINCT session_id) AS unique_sessions,
    COUNT(CASE WHEN username IS NOT NULL THEN 1 END) AS auth_attempts,
    COUNT(CASE WHEN command IS NOT NULL THEN 1 END) AS command_attempts,
    COUNT(CASE WHEN command_found = TRUE THEN 1 END) AS valid_commands,
    MIN(event_time) AS first_event,
    MAX(event_time) AS last_event
FROM cowrie_log
GROUP BY DATE(event_time), protocol;

CREATE OR REPLACE VIEW v_cowrie_attacker_behavior AS
SELECT
    source_ip,
    COUNT(*) AS total_events,
    COUNT(DISTINCT protocol) AS protocols_used,
    COUNT(DISTINCT session_id) AS sessions_created,
    COUNT(CASE WHEN username IS NOT NULL THEN 1 END) AS auth_attempts,
    COUNT(CASE WHEN command IS NOT NULL THEN 1 END) AS commands_executed,
    COUNT(CASE WHEN command_found = TRUE THEN 1 END) AS valid_commands,
    COUNT(DISTINCT username) AS usernames_tried,
    COUNT(DISTINCT fingerprint) AS unique_fingerprints,
    MIN(event_time) AS first_seen,
    MAX(event_time) AS last_seen,
    TIMESTAMPDIFF(MINUTE, MIN(event_time), MAX(event_time)) AS activity_duration_minutes
FROM cowrie_log
GROUP BY source_ip
ORDER BY total_events DESC;

CREATE OR REPLACE VIEW v_cowrie_command_statistics AS
SELECT
    command,
    COUNT(*) AS usage_count,
    COUNT(DISTINCT source_ip) AS unique_ips,
    COUNT(DISTINCT session_id) AS unique_sessions,
    command_found,
    MIN(event_time) AS first_used,
    MAX(event_time) AS last_used
FROM cowrie_log
WHERE command IS NOT NULL AND command != ''
GROUP BY command, command_found
ORDER BY usage_count DESC;

/* ========================= 蜜罐日志：MySQL ========================= */
CREATE TABLE IF NOT EXISTS mysql_honeypot_log (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    event_id VARCHAR(64) DEFAULT NULL COMMENT '事件ID',
    event_time DATETIME(6) NOT NULL COMMENT '事件时间',
    container_id VARCHAR(64) DEFAULT NULL COMMENT '容器ID',
    container_name VARCHAR(100) DEFAULT NULL COMMENT '容器名称',
    source_ip VARCHAR(45) DEFAULT NULL COMMENT '攻击源IP',
    source_port INT DEFAULT NULL COMMENT '攻击源端口',
    destination_ip VARCHAR(45) DEFAULT NULL COMMENT '目标IP',
    destination_port INT DEFAULT NULL COMMENT '目标端口',
    username VARCHAR(255) DEFAULT NULL COMMENT '登录用户名',
    password VARCHAR(255) DEFAULT NULL COMMENT '登录密码',
    database_name VARCHAR(255) DEFAULT NULL COMMENT '访问的数据库',
    query TEXT COMMENT '执行的SQL语句',
    error_code VARCHAR(50) DEFAULT NULL COMMENT '错误码',
    raw_log TEXT COMMENT '原始日志',
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录创建时间',
    PRIMARY KEY (id),
    UNIQUE KEY uk_event_id (event_id),
    KEY idx_event_time (event_time),
    KEY idx_source_ip (source_ip),
    KEY idx_username (username),
    KEY idx_container_id (container_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MySQL蜜罐日志';

CREATE OR REPLACE VIEW v_mysql_honeypot_statistics AS
SELECT
    query,
    COUNT(*) AS attempts,
    COUNT(DISTINCT source_ip) AS unique_ips,
    MAX(event_time) AS last_seen
FROM mysql_honeypot_log
WHERE query IS NOT NULL AND query != ''
GROUP BY query
ORDER BY attempts DESC;
