...
// TestAnalyzeContainerLogs 测试日志分析功能
...
未格式化的日志行，应该被识别为未知类型`
	...
	// 调用日志分析函数
	segments, stats := AnalyzeContainerLogs(logContent)
...
// TestSemanticSegmentWithMockData 测试语义分割功能（使用模拟数据）
func TestSemanticSegmentWithMockData(t *testing.T) {
	// 这个测试需要在实际环境中运行，因为它依赖于数据库和Docker
	// 这里只是提供一个示例框架
	t.Skip("跳过此测试，因为它需要实际的Docker环境")

	// 假设有一个测试容器ID
	containerID := "test-container-id"

	// 调用语义分割函数
	result, err := SemanticSegment(containerID)

	// 验证结果
	if err != nil {
		t.Errorf("语义分割失败: %v", err)
	}

	if result["success"] != true {
		t.Errorf("期望成功标志为true，实际为%v", result["success"])
	}
